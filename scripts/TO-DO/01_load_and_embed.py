"""
LAB: form product embeddings with Voyage AI.

Fill the TODOs below, then from the repo root:

    python scripts/TO-DO/01_load_and_embed.py

Solution (instructor / if you get stuck): scripts/answers/01_load_and_embed.py
...
texts: list[str] = []
for p in products:
    # Fill the field names — do not copy price or category into this string
    name = p["???"]
    tags = ", ".join(p.get("???", []) or [])
    description = p["???"]
...
for i in range(0, len(texts), batch_size):
    ...
    result = None  # TODO: vo.embed(batch, model="voyage-4-large", input_type="document")
    if result is None:
        raise NotImplementedError("TODO 2: embed this batch with Voyage AI")
...
for product, embedding in zip(products, all_embeddings):
    # TODO: product["description_embedding"] = embedding
    if "description_embedding" not in product:
        raise NotImplementedError("TODO 3: set product['description_embedding']")
